  ...
  <div class="flex-1 min-h-0 flex gap-6">
    ...
    <div class="flex-1 grid {% if slides|length <= 4 %}grid-cols-2 gap-4{% elif slides|length <= 6 %}grid-cols-3 gap-3{% else %}grid-cols-4 gap-2{% endif %} min-h-0">
      ...
      <div class="bg-gradient-to-b from-slate-900/90 to-slate-950/95 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden transition-all duration-300">
        ...
        <!-- Speaker Script script notes inside the slide preview itself to give premium look -->
        <div class="mt-3 pt-2.5 border-t border-slate-800 bg-slate-900/50 p-2 rounded-lg">
          <p class="text-[8px] font-bold text-indigo-400 uppercase tracking-wider mb-1">Speaker Script</p>
          <p class="text-[9px] text-slate-400 line-clamp-2 leading-normal italic">"{{ slide.speaker_notes }}"</p>
        </div>
...
<body class="bg-gradient-to-br {{ theme_bg }} text-slate-100 p-6 flex flex-col justify-between relative overflow-hidden">
  ...
  <!-- INFOGRAPHIC HEADER -->
  <div class="relative rounded-2xl border border-slate-800 bg-slate-900/50 p-5 backdrop-blur-md flex items-center justify-between gap-6 mb-4">
    <div class="flex-1">
  ...
  <div class="grid grid-cols-3 gap-3 mb-4">
    ...
    <div class="rounded-xl border border-slate-800/80 bg-slate-900/40 p-3 backdrop-blur-md flex flex-col justify-between">
      ...
      <!-- Premium mini bar -->
      <div class="mt-2 w-full h-1 rounded-full bg-slate-800 overflow-hidden">
  ...
  <!-- SECTIONS GRID -->
  <div class="flex-1 grid {% if sections|length <= 3 %}grid-cols-1 gap-4{% else %}grid-cols-2 gap-4{% endif %} min-h-0 mb-4">
    {% for section in sections %}
    <div class="rounded-2xl border border-slate-800 bg-slate-900/60 hover:border-slate-700/60 p-4 flex flex-col justify-between backdrop-blur-md relative overflow-hidden">
      ...
      <div>
        <div class="flex items-center gap-2 mb-3">
          ...
          <h2 class="text-xs font-bold text-white tracking-wide line-clamp-2 leading-snug">{{ section.title }}</h2>
        </div>
        
        <p class="text-[10px] text-slate-400 mb-3 leading-relaxed line-clamp-3">
          {{ section.summary }}
        </p>
      </div>
      ...
      <div class="mt-auto border-t border-slate-800 pt-3 space-y-1.5">
        {% for hl in section.highlights[:3] %}
        <div class="flex items-start gap-1.5 text-[9px] text-slate-300 leading-normal">
          <i data-lucide="check-circle-2" class="w-3 h-3 {{ icon_color }} shrink-0 mt-0.5"></i>
          <span class="line-clamp-2">{{ hl }}</span>
        </div>
        {% endfor %}
      </div>
      {% endif %}
  ...
  <div class="flex items-center justify-between border-t border-slate-850 pt-3">
    {% if call_to_action %}
    <div class="flex-1 rounded-xl border {{ cta_bg }} p-3 flex items-center gap-3 backdrop-blur-md">
      <div class="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
        <i data-lucide="sparkles" class="w-4 h-4 {{ icon_color }}"></i>
      </div>
      <div>
        <p class="text-[9px] font-bold uppercase tracking-wider {{ accent_text }}">Call to Action / สรุปปิดท้าย</p>
        <p class="text-xs text-slate-200 font-medium mt-0.5 leading-normal">{{ call_to_action }}</p>
      </div>
    </div>
    {% else %}
    ...
    <div class="text-right text-[10px] text-slate-500 pl-6 select-none shrink-0 font-mono">
...
# -------------------------------------------------------------
async def render_html_to_base64(html_content: str, width: int = 1200, height: int = 800) -> str:
    """
...
def _run_playwright_in_thread(html_content: str, width: int, height: int) -> str:
    """
    ...
    try:
        return loop.run_until_complete(render_html_to_base64(html_content, width, height))
    finally:
        loop.close()
...
async def render_action_to_image(action_type: str, data_json_str: str) -> str:
    ...
    if action_type == "slides":
        template_str = SLIDES_HTML_TEMPLATE
    ...
    # ส่งงานไปรันบน Thread แยกผ่าน run_in_executor เพื่อป้องกันการบล็อกของ SelectorEventLoop บน Windows
