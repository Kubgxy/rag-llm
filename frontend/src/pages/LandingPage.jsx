...
export default function LandingPage() {
  ...
  const entries = searchQuery.trim()
    ? categoryFiltered.filter(([, session]) =>
  ...
  const handleDragStart = (e, sessionId) => {
    ...
    const title = history[sessionId]?.title || 'แชทใหม่'
    ...
    dragElement.innerHTML = `
      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
        ...
        <div>
          ...
          <div style="color: #64748b; font-size: 11px;">กำลังย้าย...</div>
        </div>
      </div>
    ...
    e.dataTransfer.setData('text/plain', `กำลังย้าย: ${truncatedTitle}`)
  }
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide shrink-0 py-2 px-4 -mx-4">
          <div className="flex items-center gap-2 px-2">
            ...
            >
              ...
              ไม่มีหมวดหมู่ ({categoryCounts.uncategorized || 0})
            </button>
          </div>
        </div>
          ...
          <div className="space-y-2 flex-1">
            ...
            <div className="relative mt-4 max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-surface-800 border border-surface-300 dark:border-surface-700 rounded-xl text-surface-900 dark:text-white placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg transition-colors"
                >
          ...
          {entries.length === 0 ? (
            <div className="text-center py-24 bg-white/60 dark:bg-surface-900/40 rounded-3xl border-2 border-surface-200/50 dark:border-surface-800/50 border-dashed backdrop-blur-sm">
              ...
              <p className="text-surface-700 dark:text-surface-300 font-semibold text-lg">
                {searchQuery ? 'ไม่พบผลลัพธ์' : 'ยังไม่มีประวัติการแชท'}
              </p>
              <p className="text-sm text-surface-500 dark:text-surface-400 mt-2">
                {searchQuery ? `ไม่พบแชทที่ตรงกับ "${searchQuery}"` : 'กดปุ่มสร้างแชทใหม่เพื่อเริ่มต้น'}
              </p>
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors font-semibold"
                >
                  ล้างการค้นหา
                </button>
              )}
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button
                          onClick={(e) => startEditing(e, id, session.title || 'Untitled Chat')}
